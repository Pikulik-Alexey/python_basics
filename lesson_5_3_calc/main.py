...
def calc():
    ...
    if oper == '+':
        ...
        result = c.multiplication(first, second)
    elif oper == '/':
        result = c.division(first, second)
    entry.delete(0, END)
...
ttk.Button(text='+', command=lambda: set_operator('+')).grid(row=1, column=3)
ttk.Button(text='-', command=lambda: set_operator('-')).grid(row=2, column=3)
ttk.Button(text='*', command=lambda: set_operator('*')).grid(row=3, column=3)
ttk.Button(text='/', command=lambda: set_operator('/')).grid(row=4, column=3)
